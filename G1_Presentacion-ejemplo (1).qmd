...
**Relevancia:**La conectividad a internet es esencial para el desempeño académico, ya que permite el acceso eficiente a plataformas educativas. Evaluar la red en UTEC durante el ciclo 2025-I ayuda a identificar zonas críticas, optimizar recursos y mejorar la experiencia educativa.

**Contexto:**En el contexto académico actual, donde la conectividad influye directamente en el rendimiento estudiantil, resulta clave analizar cómo factores como la velocidad y latencia afectan el acceso a contenidos. Esta evaluación permite comprender los retos tecnológicos que enfrentan las instituciones educativas en un entorno cada vez más digital.

**Justificación:**Este estudio permite vincular el análisis técnico de la red con el impacto en el desempeño académico, identificando oportunidades de mejora en infraestructura y toma de decisiones. A través de un enfoque integral, se busca contribuir al desarrollo institucional mediante estrategias basadas en datos reales y situaciones concretas.
...
```{r}
vbajada_pos <- DF3$VBajada[DF3$VBajada > 0]
library(fitdistrplus)
fit.norm <- fitdist(vbajada_pos, "norm")
fit.exp  <- fitdist(vbajada_pos, "exp")
fit.lnorm <- fitdist(vbajada_pos, "lnorm")
